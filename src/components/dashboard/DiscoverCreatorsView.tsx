...
export const DiscoverCreatorsView: React.FC<DiscoverCreatorsViewProps> = ({
  ...
  // "si on a pas assez de place enlever la bannière"
    ...
    <div className="space-y-6 max-w-7xl mx-auto pb-16 text-white select-none animate-in fade-in duration-150">
      ...
      <div className="rounded-2xl border border-white/10 bg-[#121316] p-5 sm:p-6">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          {/* Banner visibility toggle (si on a pas assez de place enlever la bannière) */}
          <div className="flex items-center gap-2 self-start sm:self-auto">
            <button
              ...
              title={showBanners ? "Masquer les bannières pour une vue plus compacte" : "Afficher les bannières"}
            >
              {showBanners ? (
                <>
                  <EyeOff className="size-3.5 text-zinc-400" />
                  <span className="hidden sm:inline">Mode compact (sans bannière)</span>
                  <span className="sm:hidden">Sans bannière</span>
                </>
              ) : (
                <>
                  <Eye className="size-3.5 text-emerald-400" />
                  <span className="hidden sm:inline">Avec bannières</span>
                  <span className="sm:hidden">Avec bannières</span>
                </>
              )}
            </button>
          </div>
              ...
              <div>
                ...
                {showBanners && (
                  <div className="relative h-24 sm:h-28 w-full overflow-hidden bg-zinc-900 shrink-0">
                    ...
                    {/* Category badge */}
                    <div className="absolute top-2.5 right-2.5 bg-black/75 backdrop-blur-md px-2 py-0.5 rounded-lg text-[10px] font-bold text-zinc-300 border border-white/10">
                      {enterprise.categoryLabel}
                    </div>
                  </div>
                )}
                  ...
                  <div className="space-y-0.5">
                    <div className="flex items-center gap-1.5">
                      ...
                      {enterprise.isVerified && (
                        <ShieldCheck className="size-3.5 text-[#0055ff] shrink-0" />
                      )}
                    </div>
